...
# Check events
kubectl get events -n dclaw-seo --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-seo deployment/dclaw-seo-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-seo-db-credentials -n dclaw-seo
   ```
